...
import { 
  ...
  syncDailyNotificationsWithVisits,
...
export const AdminLayout: React.FC<AdminLayoutProps> = ({
  ...
  // Automatically sync notification history with visits prop whenever database updates
  useEffect(() => {
    if (visits && visits.length > 0) {
      const res = syncDailyNotificationsWithVisits(visits);
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {showNotificationsDropdown && (
                <div className="absolute right-0 mt-1.5 w-80 sm:w-96 bg-white text-slate-900 rounded-2xl shadow-2xl border border-slate-200 z-50 animate-fadeIn overflow-hidden text-xs">
                  ...
                  {/* Browser Desktop Notification banner if not permitted yet */}
                  {desktopPermission === 'default' && (
                    <div className="px-3 py-1.5 bg-amber-50 border-b border-amber-200 flex items-center justify-between gap-2 text-[11px]">
                      <span className="text-amber-800 flex items-center gap-1 font-medium">
                        <Laptop className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                        <span>Izin notifikasi desktop</span>
                      </span>
                      <button
                        type="button"
                        onClick={async () => {
                          const res = await requestDesktopNotificationPermission();
                          setDesktopPermission(res ? 'granted' : 'denied');
                        }}
                        className="px-2 py-0.5 bg-amber-600 hover:bg-amber-700 text-white rounded text-[10px] font-bold shrink-0 transition"
                      >
                        Aktifkan
                      </button>
                    </div>
                  ...
                  <div className="px-3.5 py-2 bg-slate-50 border-b border-slate-100 flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {/* Test Sound & Banner button */}
                      <button
                        type="button"
                        onClick={() => {
                          const test = triggerTestNotification();
                          if (visits) {
                            const res = syncDailyNotificationsWithVisits([test, ...visits]);
                            setNotificationHistory(res.visits);
                            setReadVisitIds(res.readVisitIds);
                            setUnreadCount(res.unreadCount);
                          }
                        }}
                        className="inline-flex items-center gap-1 px-2 py-1 text-slate-700 hover:text-slate-900 hover:bg-slate-200/70 rounded-lg text-[10px] font-bold transition border border-slate-200 bg-white"
                        title="Uji coba suara lonceng dan pop-up banner notifikasi"
                      >
